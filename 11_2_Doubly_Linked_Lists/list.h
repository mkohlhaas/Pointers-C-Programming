...
// List is implemented as a circualar structure with a dummy link at the beginning where next of dummy points to first
...
#define init_list_head(x)                                                                                              \
  (ListHead) {                                                                                                         \
    .prev = &(x), .next = &(x)                                                                                         \
  } /* x is a link. Can be used to create an empty List.                                         */
#define front(x)                                                                                                       \
  (x)->next /* x is a list. front returns the first real data link or the dummy link if list is empty    */
#define last(x)                                                                                                        \
  (x)->prev /* last returns the last real data link or the dummy link if list is empty                   */
#define is_empty(x) ((x) == front(x))
#define clear_head(head)                                                                                               \
  do {                                                                                                                 \
    (head) = init_list_head(head);                                                                                     \
  } while (0) /* 'head' is a link. Doesn't free links. Create dummy on the stack and copy it into 'head'.  */
#define clear_list(x)                                                                                                  \
  clear_head(*(x)) /* same for a List, which is a pointer to a head; x is a List                                */
#define insert_val_before(before, val) insert_val_after((before)->prev, val)
#define prepend insert_val_after
#define append                                                                                                         \
  insert_val_before /* returns bool                                                                              */
#define link_before(x, y)                                                                                              \
  link_after((x)->prev, y) /* place link y before link x */
...
static inline void delete_link(Link *x) {
  unlink(x);
  free(x);
}

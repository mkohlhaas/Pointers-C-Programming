...
{
  Link* plink  = malloc(sizeof *plink); if (!plink) return NULL;
  plink->value = val;
  plink->prev  = prev;
  plink->next  = next;
  return plink;
...
{
  Link* head;
  head = new_link(314159, NULL, NULL); if (!head) return NULL;  // create head/dummy
...
{
  Link* link = new_link(val, after, after->next); if (!link) return false;
...
{
  List x = new_list(); if (!x) return NULL;

  for (int i = 0; i < n; i++)
    if (!append(x, array[i])) { free_list(x); return NULL; }  // append after head
  return x;
}
...
// We don't delete y, but we empty it.  The caller must free it if he no longer needs
// it. We could free it here, but that just changes the API. It is a design choice.
void
...
copy_list(List x)
{
  List newlist = new_list(); if (!newlist) return NULL;
  for (Link* p = front(x); p != x; p = p->next)
    if (!append(newlist, p->value)) { free_list(newlist); return NULL; }
  return newlist;
}

...
// Warn of unused function results as Point and Rectangle are not changed in-place.
...
Point move_point_horizontally(Point p, double amount) {
  p.x += amount;
  return p;
}
...
void print_point(Point p) { printf("point <%.2f, %.2f>\n", p.x, p.y); }

///////////////
// Rectangle //
///////////////

Rectangle move_rectangle(Rectangle rect, double delta_x, double delta_y) {
...
int main() {
  ...
  print_point(p);
  printf("------------------------------\n");
  Rectangle rect = {.upper_left = {.x = 0.0, .y = 10.0}, .lower_right = {.x = 10.0, .y = 0.0}};

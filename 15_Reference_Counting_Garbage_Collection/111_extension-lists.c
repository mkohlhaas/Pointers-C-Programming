...
length_rec(takes List x_, int acc)
{
  ...
  else              return length_rec(incref(x->next), acc + 1);
}
...
reverse_rec(takes List x_, takes List acc_)
{
  ...
  if      (is_error(x) || is_error(acc)) return NULL;
  else if (is_nil(x))                    return incref(acc);
  else                                   return reverse_rec(incref(x->next), cons(x->value, incref(acc)));
}
...
concat(takes List x_, takes List y_)
{
  ...
  if      (is_error(x) || is_error(y))  return NULL;
  else if (is_nil(x))                   return incref(y);
  else                                  return cons(x->value, concat(incref(x->next), incref(y)));
}

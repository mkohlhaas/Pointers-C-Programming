...
new_pool(size_t init_capacity)
{
  ...
  if (!subpool) { free(pool); return NULL; }
...
main()
{
  ...
  Node* n2  = node_alloc(pool); assert(n2  - n1 == 1);
  Node* n3  = node_alloc(pool); assert(n3  - n2 == 1);
  Node* n4  = node_alloc(pool); assert(n4  - n3 != 1);  // Technically they could sit like that, but it is highly unlikely...
  Node* n5  = node_alloc(pool); assert(n5  - n4 == 1);
  Node* n6  = node_alloc(pool); assert(n6  - n5 == 1);
  Node* n7  = node_alloc(pool); assert(n7  - n6 == 1);
  Node* n8  = node_alloc(pool); assert(n8  - n7 == 1);
  Node* n9  = node_alloc(pool); assert(n9  - n8 == 1);
  Node* n10 = node_alloc(pool); assert(n10 - n9 != 1);  // Technically they could sit like that, but it is highly unlikely...

  free_node(pool, n2);
  free_node(pool, n10);
  Node* reused10 = node_alloc(pool); assert(n10 == reused10);
  Node* reused2  = node_alloc(pool); assert(n2  == reused2);

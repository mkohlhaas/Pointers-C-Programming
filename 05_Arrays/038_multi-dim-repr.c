...
main()
{
  ...
  int* p = (int*)A;
  for   (int i = 0; i < 2; i++) { assert( p ==  A[i]);     // p points to the first element in row i
    for (int j = 0; j < 3; j++) { assert( p ==  A[i]+j);   // p points to j-th column in i-th row
                                  assert( p == &A[i][j]);
                                  assert(*p ==  A[i][j]);
                                  p++; } }                 // incrementing p means going to next column
  ...
  p = (int*)B;
  for     (int i = 0; i < 2; i++) { assert( p == (int*) B[i]);        // p points to row i
    for   (int j = 0; j < 2; j++) { assert( p == (int*)(B[i]+j));     // p points to j-th column in i-th row
      for (int k = 0; k < 3; k++) { assert( p ==        B[i][j]+k);   // p points to k-th element in B[i][j]
                                    assert( p ==       &B[i][j][k]);
                                    assert(*p ==        B[i][j][k]);
                                    p++; } } }

  char x[4][3] = { { 'a', 'b', 'c' }, // complete row
                   { 'd', 'e' },      // first 2 columns

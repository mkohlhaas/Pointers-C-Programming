...
void push(FreeFrame **stack, STree tree) {
  if (!tree)
    return;
  FreeFrame *frame = malloc(sizeof *frame);
  if (!frame)
    abort(); // we do not tolerate errors! Allocate a FreeFrame structure
...
bool insert(STree *tree, int val) {
  ...
  if (*target)
    return true; // we have already been here
  else
...
typedef enum print_op { LPAR, TREE, COMMA, VALUE, RPAR } PrintOp;
...
void print_stree(STree *tree) {
  ...
  if (setjmp(stack.env) != 0)
    goto error; // save cpu registers (p.395)

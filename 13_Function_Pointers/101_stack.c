...
typedef void* ImplStack;  // pointer to the data structure; as it could be anything, e.g. a list, a dynamic array, we use a void pointer
...
// this is the central structure that we get when calling new_stack() and is a parameter for every stack function, eg. stack_pop(), stack_push(), ...
...
error:
  free(stack);
  if (impl_stack) type->free_stack(impl_stack);
  return NULL;
}
...
{
  return stack->type->empty_stack(stack->implementation);
}

Elem
stack_top(Stack stack)
{
  return stack->type->top(stack->implementation);
}

bool
stack_push(Stack stack, Elem elem)
{
  return stack->type->push(stack->implementation, elem);
}

Elem
stack_pop(Stack stack)
{
  return stack->type->pop(stack->implementation);
}
...
bool
list_stack_empty(ImplStack stack)
{
  return is_empty((List)stack);
}

Elem
list_stack_top(ImplStack stack)
{
  return front((List)stack)->value;
}

bool
list_stack_push(ImplStack stack, Elem elem)
{
  return prepend(stack, elem);
}

Elem
list_stack_pop(ImplStack stack)
{
...
void
list_stack_free(ImplStack stack)
{
  free_list(stack);
}
...
bool
da_stack_empty(ImplStack stack)
{
  return ((Dynarray*)stack)->used == 0;
}

Elem
da_stack_top(ImplStack stack)
{
  Dynarray* da = stack;
  assert(da->used > 0);
...
bool
da_stack_push(ImplStack stack, Elem elem)
{
  return da_append(stack, elem);
}

Elem
da_stack_pop(ImplStack stack)
{
  Dynarray* da = stack;
  assert(da->used > 0);
...
test(StackType* type)
{
  ...
  while (!empty_stack(stack)) printf("%d\n", stack_pop(stack));
  free_stack(stack);
}

...
#define checked_malloc(n, type)                                                \
  (size_check((n), (type)) ? malloc((n) * sizeof(type)) : NULL)
#define checked_realloc(p, n, type)                                            \
  (size_check((n), (type)) ? realloc((p), (n) * sizeof(type)) : NULL)
...
#define at_max_len(n, type) ((n) == max_array_len(type))

#define capped_dbl(n, type)                                                    \
  (((n) < max_array_len(type) / 2) ? (2 * (n)) : max_array_len(type))

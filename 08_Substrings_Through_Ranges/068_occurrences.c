...
int main() {
...

  {
    ...
    for (Substr occ = next_occurrence(&iter, y, 0); !is_null_substr(occ); occ = next_occurrence(&iter, y, 0)) {
...

  {
    ...
    for (Substr occ = next_occurrence(&iter, y, 1); !is_null_substr(occ); occ = next_occurrence(&iter, y, 1)) {
      printf("Found an occurrence at index %ld\n", occ.begin - x.begin);
    }
  }
}

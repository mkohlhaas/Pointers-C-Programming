...
substr_cmp (Substr x, Substr y)
{
  ...
  if (x.begin < x.end)
    { // x is the longer string
      return +1;
    }
  if (y.begin < y.end)
    {       // y is the longer string
      return -1;
    }
...
Substr
next_occurrence (SubstrIter *iter, Substr s, bool overlaps)
{
...
Substr
first_occurrence (SubstrIter *iter, Substr s, bool overlaps)
{
  return next_occurrence (iter, s, overlaps);
}

...
Substr
slice(char* s, size_t begin, size_t end)
{
  assert(begin <= end);
...
char*
substr_to_buf(char* to, Substr from)
{
  while (from.begin < from.end) *to++ = *from.begin++;
  *to = '\0';
  return to;
}

void
print_substr(Substr s)
{
  while (s.begin < s.end) putchar(*s.begin++);
}

// Mutating substrings to create strings
char
insert_zero_term(Substr s)
{
  char c = *s.end;
  *s.end = '\0';
  return c;
...
substr_cmp(Substr x, Substr y)
{
  ...
  if (x.begin < x.end) return +1;
  if (y.begin < y.end) return -1;
  return 0;
}
...
// the end of to.
Substr
copy_substr(Substr to, Substr from)
{
...
// the end of out.
Substr
replace_substr(Substr out, Substr z, Substr x, Substr y)
{
...
Substr
replace_substr_inplace(Substr z, Substr x, Substr y)
{
...
Substr
insert_substr(Substr out, Substr z, size_t index, Substr y)
{
...
Substr
insert_substr_inplace(Substr x, size_t index, Substr y)
{
...
Substr
delete_substr(Substr out, Substr x, Substr y)
{
...
Substr
delete_substr_inplace(Substr x, Substr y)
{
...
Substr
next_word(SubstrIter* iter)
{
  char* begin = find_word(iter->begin);
  if (*begin == '\0') return NULL_SUBSTR; // no more words
...
Substr
copy_words(Substr to, Substr from)
{
  char*      begin = to.begin;
...
Substr
find_occurrence(Substr x, Substr y)
{
  int n = substr_len(x);
  int m = substr_len(y);
  ...
  char* s   = x.begin;
  char* end = x.end - m + 1;
  for (; s < end; s++) if (!strncmp(s, y.begin, m)) return SUBSTR(s, s + m);
  return NULL_SUBSTR;
}

// Iterator for non-overlapping occurrences of s
Substr
next_occurrence(SubstrIter* iter, Substr s, int overlaps)
{
  Substr occ = find_occurrence(*iter, s);
  // if there was an occurrence, we must update the iterator
  if (!is_null_substr(occ)) iter->begin = overlaps ? occ.begin + 1 : occ.end;
  return occ;
}

// Dynamic array is allocated on the heap as a generic memory buffer.
// Using `char*` as a generic byte pointer.
...
typedef struct
{
  size_t size;
  size_t used;
  size_t obj_size; // Update: added size of objects (we have to do pointer arithmetic by hand)
...
da_append (dynarray *da, void *val)
{
  if (da->used == da->size)
    {
      if (is_at_max_len (da->size, da->obj_size))
        {
          return false;
        }
...
void
da_print (dynarray *da)
{
  for (int i = 0; i < da_len (da); i++)
    {
      printf ("%d ", da_at_as (da, i, int)); // da_at_as used as right-value
    }
  printf ("\n");
}
...
main ()
{
  ...
  {
    // init dynamic array (include sizeof here)
...

  {
    ...
    da_append (&da, &(int){ 314 }); // compound literal is on the stack but will be copied in da_append
    da_print (&da);
  }

  {
    ...
    for (int i = 0; i < 5; i++)
      {
        da_at_as (&da, i, int) += 10; // da_at_as macro can also be used as left-value
      }
    da_print (&da);
  }

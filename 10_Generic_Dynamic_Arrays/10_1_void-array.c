...
typedef struct {
  ...
  size_t used;
  void **data; // Update int* => void** (but we store void* in data and cast it); array of ints -> array of void
               // pointers; we cannot have array of voids
} dynarray;
...
bool da_resize(dynarray *da, size_t new_size) {
...
bool da_append(dynarray *da, void *val) {
  if (da->used == da->size) {
...
int main() {
...

  {
    ...
    for (int i = 0; i < 5; ++i)
      da_append(&da,
                &(int){i}); // These are all the same local integer! We are filling the array with five copies of 4.
    printf("Address of loop variable: %p\n", da.data[da.used - 1]);
    for (int j = 0; j < 5; ++j)
      da_append(&da,
                &(int){j}); // These are all the same local integer! We are filling the array with five copies of 4.
...
#define da_get_deref(da, i, type) *(type *)(da_at((da), (i)))
  ...
  print_da(&da);
  putchar('\n');
  printf("%p\n", da.data); // this has been first allocated in da_init, but could have been changed later when resizing
                           // the dynamic array
  da_dealloc(&da);
}

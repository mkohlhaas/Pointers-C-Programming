...
// ========== Pointer Dynarray =======================================================
typedef struct { double x, y; } point;                   // typedefs for custom types
...
{
  {
    ...
    for (int i = 0; i < da_len(&da); i++) da_at(&da, i) = i;             // insert some values into dynarray
    for (int i = 0; i < da_len(&da); i++) printf("%d ", da_at(&da, i));  // print dynarray
    printf("\ncurrent length: %zu\n", da_len(&da));
...

  {
    ...
    if (!success) printf("allocation error\n");
    for (int i = 0; i < da_len(&dda); i++) da_at(&dda, i) = (double)i;       // insert some values into dynarray
    for (int i = 0; i < da_len(&dda); i++) printf("%.1f ", da_at(&dda, i));  // print dynarray
    printf("\ncurrent length: %zu\n", da_len(&dda));
    for (int i = 0; i < 10; i++)                                             // append more values into dynarray
...

  {
    ...
    for (int i = 0; i < 5; i++) point_da_append(&pda, (point){ .x = i, .y = i });   // insert some values into dynarray

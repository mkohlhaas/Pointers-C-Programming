...
STreeType strnode_type = { .key   = strnode_key,
                           .cmp   = strnode_cmp,
                           .print = strnode_print,
                           .free  = strnode_free };
...
ListType strlink_type = { .print = strlink_print,
	                  .free  = strlink_free };
...
{
  for (Link* lnk = front(x); lnk != head(x); lnk = lnk->next) if (idx-- == 0) return lnk;
  return NULL;
}
...
{
  for (Link* lnk = back(x); lnk != head(x); lnk = lnk->prev)  if (idx-- == 0) return lnk;
  return NULL;
}
...
{
  Link* lnk;
  if   (idx < 0)  lnk = take_back (os->order, -idx - 1);
  else            lnk = take_front(os->order,  idx);
  if (!lnk) return;  // report an error...

...
STree *new_tree(STreeType type) {
  ...
  if (!(type.key && type.cmp))
    return NULL;
  STree *tree = malloc(sizeof *tree);
  if (tree)
    *tree = (STree){.root = {.parent = NULL, .left = NULL, .right = NULL}, .type = type};
  return tree;
}

// Find parent and child
Node **find_loc(STree *tree, void const *key, Node **n, Node **p) {
...
void insert_node(STree *tree, Node *n) {
  ...
  if (*target)
    delete_node(tree, *target); // remove the old node and replace it with the new one
  *target = n;
  n->parent = parent;
...
static void default_print(Node *n) { printf("<node %p>", n); }

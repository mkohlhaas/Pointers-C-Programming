...
List*
new_list(ListType type)
{
  List* list = malloc(sizeof *list);
  if (list) *list = (List){ .head = { .next = &list->head, .prev = &list->head },
                            .type = type };
  return list;
}
...
Link*
find_link(List* x, Link* from, bool (*p)(Link*))
{
  for (Link* lnk = from; lnk != head(x); lnk = lnk->next) if (p(lnk)) return lnk;
  return NULL;
}

void
delete_if(List* x, bool (*p)(Link*))
{

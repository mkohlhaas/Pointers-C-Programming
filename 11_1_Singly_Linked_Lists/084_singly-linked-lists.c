...
make_list(int n, int array[n])
{
  ...
  for (int i = n - 1; i >= 0; i--) {
    ...
    if (!newlink) { free_list(list); return NULL; }
    list = newlink;
  }
  return list;
}
...
append(List* list, int val)
{
  ...
  if (!val_link) { free_list(list); return NULL; }
...
main()
{
...

  {
    ...
    if (!list) { perror("List error: "); exit(1); }
    printf("Contains:\n");
...

  {
    ...
    if (!new_list) { perror("List error: "); exit(1); }
    list = new_list;
...

  {
    ...
    if (!list2) { perror("List error: "); exit(1); }

...
make_list(int n, int array[n])
{
  ...
  for (int i = n - 1; i >= 0; i--) {
    ...
    if (!link) { free_list(x); return NULL; }
...
print_list(List x)
{
  ...
  while (link) { printf("%d ", link->value); link = link->next; }
  printf("]\n");
}
...
delete_value(List x, int val)
{
  ...
  while (front) {
    while ((next = front->next) && next->value == val) { // first expression will always be evaluated; will be NULL at end of list
...
main()
{
...

  {
    ...
    List x = make_list(n, array); if (!x) { perror("Make list error:"); exit(2); }
    print_list(x);
...

  {
    ...
    List x = make_list(n, array); if (!x)  { perror("Make list error:"); exit(1); }

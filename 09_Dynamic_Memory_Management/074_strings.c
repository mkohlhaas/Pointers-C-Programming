...
char const*
slice(char const* x, int begin, int end)
{
  assert(begin <= end);
...
#define size_check_inc_product(var, x, y)                                      \
  {                                                                            \
    if ((x) > 0 && (SIZE_MAX - (var)) / (x) < (y)) goto error;                 \
    (var) += (x) * (y);                                                        \
  }

// replace substring from 'begin' to 'end' in 'x' with 'y'
char*
replace_string(char const* x, int begin, int end, char const* y)
{
...
char*
insert_string(char const* x, int begin, char const* y)
{
  return replace_string(x, begin, begin, y);
}

char*
delete_string(char const* x, int begin, int end)
{
  return replace_string(x, begin, end, "");
}

char*
join_strings(char const* sep, int n, char const* strings[n])
{
...
#define append_string(src) { for (char const* p = src; *p; p++) *dst++ = *p; }
...
char*
replace_all_occurrences(char const* z, char const* x, char const* y)
{
  ...
  SubstrIter iter = ssz;
  for (Substr occ = next_occurrence(&iter, ssx, 0);   // 0 = non-overlapping
              !is_null_substr(occ);
              occ = next_occurrence(&iter, ssx, 0)) { // 0 = non-overlapping
    len -= xlen;
...
#define copy_range(b, e) { for (char const* p = (b); p != (e); p++) *dst++ = *p; }

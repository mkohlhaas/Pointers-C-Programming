...
void print_array(int *begin, int *end) {
  while (begin < end)
    printf("%2d ", *begin++); // begin will be incremented after calling printf
  putchar('\n');
}
...
int eratosthenes(int n, int buf[n - 2]) {
  ...
  for (int i = 0; i * i < n - 2; i++) {
    ...
    int p = buf[i];
    for (int j = p * p; j < n;
         j += p) // p * p b/c we had already sieved all the numbers < p, e.g. 2 * p, 3 * p, 5 * p if p == 7
      buf[j - 2] = NOT_PRIME;
  }
...
void sieve_candidates__(int **from, int **to, int p) {
...
int eratosthenes__(int n, int buf[n - 2]) {
  ...
  while (candidates < end) {
    if ((*candidates) * (*candidates) > n)
      break; // parens not needed but easier to read
    print_array(buf, buf + n - 2);
    int p = *candidates++;
...
void sieve_candidates___(int **from, int **to) {
  int p = *(*from)++;
  int *output = *from;
  for (int *input = *from; input < *to; input++)
    if (*input % p != 0)

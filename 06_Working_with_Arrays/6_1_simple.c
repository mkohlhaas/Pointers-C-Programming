...
void add_pointers(int *start, int *end, int x) {
  while (start < end) {
    *start++ += x;
  }
}

void add_pointers1(int *start, int *end, int x) {
  for (; start < end; start++) {
    *start += x;
  }
}
...
void swap_pointers(int *left, int *right) {
  // printf("i: %p, j: %p\n", i, j);
  int temp;
  temp = *left;
  *left = *right;
...
int sum_pointers(int *start, int *end) {
  int sum = 0;
  while (start < end) {
    sum += *start++;
  }
...
int sum_pointers1(int *start, int *end) {
  int sum = 0;
  for (; start < end; start++) {
    sum += *start;
  }

...
void
swap_down(char* start, char* current, size_t obj_size, CompareFN cmp)
{
  while (current != start) {
...
void
insertion_sort(void* array, size_t len, size_t obj_size, CompareFN cmp)
{
  char* start = array;
  // array of sizes 0 and 1 are sorted by definition
  for (int i = 1; i < len; i++) swap_down(start, start + i * obj_size, obj_size, cmp);
}
...
main()
{
  ...
  for (int i = 0; i < int_array_length; i++) printf("%d ", int_array[i]);
  putchar('\n');
  ...
  for (int i = 0; i < string_array_length; i++) printf("%s ", string_array[i]);
  putchar('\n');
}

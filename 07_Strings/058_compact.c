...
{
  while (isalpha(**from)) *(*to)++ = *(*from)++;
}

void
copy_words(char* from, char* to)
{
  from = find_word(from);
  while (*from) {
    copy_word(&from, &to);   // from,to will be advanced
    from = find_word(from);  // find next word
    if (*from) *to++ = ' ';  // insert space
  }
  *to = '\0';
}

void
compact_words(char* s)
{
...
main()
{
  ...
  char words[] = "First  \t asdlfkj  \t   34  \t  2345\t  \t  2345  \t  sdfk  \t  kj \t   230 \t   asdo  \t  13  \t  aoanq \t   2424 \t   eqycp";
